...
<a href="/index.php/Home/bestforbaby">首页</a>
 <br/>
 <?php if(is_array($items)): foreach($items as $key=>$t): echo ((isset($t["view"]) && ($t["view"] !== ""))?($t["view"]):"默认外观"); ?>
 <br/><?php endforeach; endif; ?>
<form method="post" action="/index.php/Home/Bestforbaby/insertview" name="f1" enctype="multipart/form-data">
  <input name="item_id" value="<?php echo ($item_id); ?>" style="display:none;"/>
  <br/>
  <label for="view">输入外观描述：</label>
  <input id="view" name="view" placeholder="默认外观" value="" />
  <br/>
