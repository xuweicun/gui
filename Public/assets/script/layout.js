...
$(function(){
    $('#txtKeywords').bind('keypress',function(event){
        if(event.keyCode == "13"){
            c_search($('#txtKeywords').val());
        }
    });
    $('#lbtnSearch').bind('click',function(event){
        c_search($('#txtKeywords').val());
    });
...
$.fn.ruleSingleSelect = function () {
    var singleSelect = function (parentObj) {
        ...
        $(document).click(function (e) {
            selectObj.trigger("blur"); //触发select的onblur事件
            arrowObj.hide();
            itemObj.hide(); //隐藏下拉框
        });
    };
...
function to_search(type,keyword){
    host='http://'+top.location.hostname+'/search/'+type+'.r?keyword='+keyword;
